...
export class PublicOfferService {
    ...
    private formatOffer(offer: NonNullable<Awaited<ReturnType<typeof publicOfferRepository.findByTokenFull>>>) {
        return {
            ...
            status: offer.status,
            templateType: offer.templateType ?? 'classic',
            blocks: offer.blocks,
            ...
            seller: {
                ...
                logoLight: offer.user.companyInfo?.logoLight ?? offer.user.companyInfo?.logo ?? null,
                logoDark: offer.user.companyInfo?.logoDark ?? null,
