...
import type { WebsiteV2Blocks, WebsiteV2PageBreakKey, WebsiteV2SectionKey } from '@/lib/pdf/website-v2-blocks'
...
export function WebsiteV2SectionManagerPanel({ blocks, onSave, onClose }: WebsiteV2SectionManagerPanelProps) {
    ...
    const togglePageBreakAfter = (key: WebsiteV2PageBreakKey) => {
        const nextPageBreakAfter = pageBreakAfter.includes(key)
            ? pageBreakAfter.filter(k => k !== key)
            : [...pageBreakAfter, key]
        setDraft({ ...draft, pageBreakAfter: nextPageBreakAfter })
    }

    const remove = (key: WebsiteV2SectionKey) => setDraft({
        ...draft,
        sections: sections.filter(k => k !== key),
            ...
            <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-2">
                <div className="rounded-lg border border-border bg-background px-3 py-2">
                    <div className="flex items-center gap-2">
                        <span className="flex-1 text-sm font-medium text-foreground">{BLOCK_LABELS.cover}</span>
                    ...
                    <label className="mt-2 flex cursor-pointer items-center gap-2 text-xs text-muted-foreground">
                        ...
                        />
                        <span>Po tej sekcji nowa strona</span>
                    </label>
                </div>
                {sections.map((key, i) => {
                    const block = draft[key] as { enabled?: boolean }
