...
interface ClientCardProps {
    recipient: { id: string; name: string; email: string; type: 'client' | 'lead' };
}

export function ClientCard({ recipient }: ClientCardProps) {
    const router = useRouter();
    const tr = useTranslations('offerDetail');
    const href = recipient.type === 'client'
        ? `/dashboard/clients/${recipient.id}`
        : `/dashboard/leads/${recipient.id}`;
