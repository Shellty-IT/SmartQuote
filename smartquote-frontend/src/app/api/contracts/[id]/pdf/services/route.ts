...
export async function GET(
    ...
    let html: string
    try {
        html = addDocumentActionLinks(buildContractServicesHtmlFromSaved(contract.blocks, { editorMode: false }), publicDocumentUrl('contract', contract.publicToken, 'sign'), 'sign')
    } catch (err) {
        const detail = err instanceof Error ? `${err.message}\n${err.stack}` : String(err)
        console.error('[contract-services-pdf] buildContractServicesHtmlFromSaved threw:', detail)
        return new Response(
    ...
    try {
        ...
        const detail = err instanceof Error ? `${err.message}\n${err.stack}` : String(err)
        console.error('[contract-services-pdf] Puppeteer error:', detail)
        return new Response(
    ...
    const filename = `Umowa_${number.replace(/\//g, '-')}_sklep.pdf`
