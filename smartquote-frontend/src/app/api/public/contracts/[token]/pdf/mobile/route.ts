...
export async function GET(
    ...
    const html = buildHtmlOrRespond('public-contract-mobile-pdf', () =>
        addDocumentActionLinks(buildContractMobileHtmlFromSaved(contract.blocks, { editorMode: false }), publicDocumentUrl('contract', token, 'sign'), 'sign'),
    )
    if (html instanceof Response) return html

    const buffer = await renderPdfOrRespond('public-contract-mobile-pdf', html)
