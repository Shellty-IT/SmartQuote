...
export default function LoginPage() {
    ...
    const handleLogin = async () => {
        ...
        try {
            ...
            if (result?.error) { setError(tr.login.errorInvalid); setIsLoading(false); return; }
            if (result?.ok) router.push('/dashboard');
        } catch {
            setError(tr.login.errorConnection);
            setIsLoading(false);
        }
    };
    ...
    const statusBanner: Record<BackendStatus, { icon: ReactNode; text: string; cls: string }> = {
        checking: { icon: <Loader2 className="h-4 w-4 animate-spin" />, text: tr.login.statusChecking, cls: 'bg-primary/10 text-primary border-primary/20' },
        waking:   { icon: <Loader2 className="h-4 w-4 animate-spin" />, text: `${tr.login.statusWaking} (${elapsedSeconds}s)`, cls: 'bg-primary/10 text-primary border-primary/20' },
        ready:    { icon: <CheckCircle2 className="h-4 w-4" />, text: tr.login.statusReady, cls: 'bg-status-accepted/10 text-status-accepted border-status-accepted/20' },
        error:    { icon: <AlertCircle className="h-4 w-4" />, text: tr.login.statusError, cls: 'bg-destructive/10 text-destructive border-destructive/20' },
    };
            ...
            <div className="relative w-full max-w-md">
                <div className="rounded-2xl border border-border bg-card p-8 shadow-elevated">
                    ...
                    <div className={cn('mb-6 flex items-center gap-3 rounded-xl border px-3 py-2.5 text-sm font-medium transition-all', banner.cls)}>
                        ...
                        {backendStatus === 'waking' && <span className="text-xs opacity-70">{tr.login.upTo}</span>}
                        {backendStatus === 'error' && (
                            <button onClick={handleRetry} className="shrink-0 text-xs underline opacity-80 hover:opacity-100">{tr.login.retry}</button>
                        )}
                    </div>

                    {backendStatus === 'waking' && (
                        <div className="mb-6 h-1.5 overflow-hidden rounded-full bg-secondary">
                            <div className="h-full rounded-full bg-gradient-primary transition-all duration-1000" style={{ width: `${Math.min((wakeAttempt / 12) * 100, 95)}%` }} />
                        </div>
                    )}
                    ...
                    <form onSubmit={(e) => { e.preventDefault(); void handleLogin(); }} className="space-y-4">
                        ...
                        >
                            {isLoading
                                ? <><Loader2 className="h-4 w-4 animate-spin" /> {tr.login.loggingIn}</>
                                : backendStatus !== 'ready'
                                    ? tr.login.waitingServer
                                    : tr.login.loginBtn
                            }
                        </button>
                    </form>
